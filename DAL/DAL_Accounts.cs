...
namespace DAL
{
    public class DAL_Accounts : DAL
    {
        public DataTable Crud_Accounts(int OperationId, int? PageNumber, int? PageSize, int? AccountId, string AccountName, bool? IsActive, int? UserId, string UserIp)
        {
            DataTable dt = new DataTable();
            try
            {
                using (SqlCommand cmd = new SqlCommand("Crud_Accounts"))
                {
                    OpenConnection(true);
